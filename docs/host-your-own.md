# Deployment Notes
The easiest way to get ShareGlimmer up and running is by using Docker.
...
```bash
docker run -d --restart=unless-stopped --name=ShareGlimmer -p 127.0.0.1:3000:3000 lscr.io/linuxserver/ShareGlimmer
```
...
-e WS_FALLBACK=true
```
> Provides ShareGlimmer to clients with an included websocket fallback if the peer to peer WebRTC connection is not available to the client.
>
> This is not used on the official https://ShareGlimmer.net, but you can activate it on your self-hosted instance.
...
> Specify the STUN/TURN servers ShareGlimmer clients use by setting `RTC_CONFIG` to a JSON file including the configuration.
> You can use `ShareGlimmer/rtc_config_example.json` as a starting point.
>
...
> 
> If this flag is set to `"true"` each peer that connects to the ShareGlimmer server will produce a log to STDOUT like this:
> ```
> ----DEBUGGING-PEER-IP-START----
> remoteAddress: ::ffff:172.17.0.1
> x-forwarded-for: 19.117.63.126
> cf-connecting-ip: undefined
> ShareGlimmer uses: 19.117.63.126
> IP is private: false
> if IP is private, '127.0.0.1' is used instead
> ----DEBUGGING-PEER-IP-END----
> ```
> If the IP ShareGlimmer uses is the public IP of your device everything is correctly setup. 
...
```bash
docker run -d --restart=unless-stopped --name=ShareGlimmer -p 127.0.0.1:3000:3000 ghcr.io/schlagmichdoch/ShareGlimmer npm run start:prod 
```
> You must use a server proxy to set the X-Forwarded-For to prevent all clients from discovering each other (See [#HTTP-Server](#http-server)).
>
> To prevent bypassing the proxy by reaching the docker container directly, `127.0.0.1` is specified in the run command.
...
#### Build the image
```bash
docker build --pull . -f Dockerfile -t ShareGlimmer
```
...
#### Run the image
```bash
docker run -d --restart=unless-stopped --name=ShareGlimmer -p 127.0.0.1:3000:3000 -it ShareGlimmer npm run start:prod
```
> You must use a server proxy to set the X-Forwarded-For to prevent all clients from discovering each other (See [#HTTP-Server](#http-server)).
>
> To prevent bypassing the proxy by reaching the docker container directly, `127.0.0.1` is specified in the run command.
...
services:
    ShareGlimmer:
        image: lscr.io/linuxserver/ShareGlimmer:latest
        container_name: ShareGlimmer
...
```bash
git clone https://github.com/schlagmichdoch/ShareGlimmer.git && cd ShareGlimmer
```
...
```
> Specify the port ShareGlimmer is running on. (Default: 3000)
...
> You must use a server proxy to set the X-Forwarded-For to prevent all clients from discovering each other (See [#HTTP-Server](#http-server)).
>
> Use this when deploying ShareGlimmer with node to prevent bypassing the proxy by reaching the docker container directly.
...
## HTTP-Server
When running ShareGlimmer, the `X-Forwarded-For` header has to be set by a proxy. Otherwise, all clients will be mutually visible.
...
server {
    ...
    ssl_certificate /etc/ssl/certs/ShareGlimmer-dev.crt;
    ssl_certificate_key /etc/ssl/certs/ShareGlimmer-dev.key;
...
```bash
    git clone https://github.com/schlagmichdoch/ShareGlimmer.git
...
- To debug the NodeJS server run `docker logs ShareGlimmer_node_1`.
